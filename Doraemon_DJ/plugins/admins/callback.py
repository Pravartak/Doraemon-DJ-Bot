...
from Doraemon_DJ.misc import SUDOERS, db
from Doraemon_DJ.utils.database import (is_active_chat,
...
from Doraemon_DJ.utils.formatters import seconds_to_min
from Doraemon_DJ.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Dora.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await Dora.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await Dora.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        check = db.get(chat_id)
        txt = f"TᖇᗩᑕK ՏKIᑭᑭᗴᗪ ᗷY {mention}"
        popped = None
        try:
            popped = check.pop(0)
            if popped:
            ...
            if not check:
                await CallbackQuery.edit_message_text(
                    f"TᖇᗩᑕK ՏKIᑭᑭᗴᗪ ᗷY {mention}"
                )
                await CallbackQuery.message.reply_text(
                    _["admin_10"].format(mention)
                )
                try:
                    return await Dora.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Dora.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Dora.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
        ...
        if int(command) in [1, 3]:
            if (duration_played - duration_to_skip) <= 10:
                ...
                return await CallbackQuery.answer(
                    f"ᗷOT IՏ ᑎOT ᗩᗷᒪᗴ TO ՏᗴᗴK ᗪᑌᗴ TO TOTᗩᒪ ᗪᑌᖇᗩTIOᑎ ᕼᗩՏ ᗷᗴᗴᑎ ᗴ᙭ᑕᗴᗴᗪᗴᗪ.\n\nᑕᑌᖇᖇᗴᑎTᒪY ᑭᒪᗩYᗴᗪ** {bet}** ᗰIᑎᑌTᗴՏ OᑌT Oᖴ **{duration}** ᗰIᑎᑌTᗴՏ",
                    show_alert=True,
                )
